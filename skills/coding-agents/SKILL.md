...
- `<git-root>/.coding-agents/assignments.md`: fixed 14-role assignment scaffold. Each scaffold section must include `role`, `status`, `task_id`, `epoch`, `scope`, `assignment`, `expected_output`, and `lifecycle`; it is not proof that 14 workers are active. Debug or repair tasks must also carry the debugging integrity gate, and gate-required work must carry the Meta-Cognitive Debug/Repair Gate.
...
6. Initialize the fixed 14-role assignment scaffold with `task_id`, `epoch`, `scope`, and `lifecycle`; create actual specialist assignments only when scoped work is dispatched.

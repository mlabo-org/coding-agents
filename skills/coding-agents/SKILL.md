...
- This skill does not authorize the parent to call `wait_agent` after spawning or assigning subagents. The global `wait_agent` boundary applies to Coding Agents work: unless the user explicitly asks for synchronous waiting, the parent returns the assignment state, held next action, and resume condition instead of synchronously collecting subagent results.
...
9. Ensure generated assignments, runner prompts, runner packets, and handoff material carry the lifecycle rule: subagents return concise integration material, stop waiting for more work, are closed or retired promptly when no longer needed, and do not require parent-side `wait_agent` unless the user explicitly asked for synchronous waiting.

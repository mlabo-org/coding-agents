...
- Use this skill only when the user explicitly asks for the `coding-agents` plugin, Coding Agents workflow, source CLI, subagent development team coordination, `.coding-agents` workflow state, task_id/epoch/scope assignments, assign/collect/run orchestration, handoff generation, workflow audit, or legacy `docs/codex` migration.
...
- `<git-root>/.coding-agents/assignments.md`: 14 role assignments. Each role must include `role`, `status`, `task_id`, `epoch`, `scope`, `assignment`, and `expected_output`.
- `<git-root>/.coding-agents/handoff.md`: prompt material for the next worker to continue the task.
...
8. Treat marketplace registration, `~/.codex/plugins/cache/` refresh, and Codex restart/new-thread activation as separate work unless the user explicitly includes them.
...
5. Initialize the empty specialist warm pool and define the first 14 role assignments with `task_id`, `epoch`, and `scope`.
...
9. Verify implementation against the accepted decisions and completion conditions.
10. Append audit results to `.coding-agents/audit.md`, including checks not run and why.
11. Report final status with changed files, verification, open risks, and next TODOs.

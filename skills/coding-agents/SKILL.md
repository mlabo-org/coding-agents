...
10. Ensure generated assignments, runner prompts, runner packets, and handoff material carry the nested Coding Agents preflight suppression rule, finite hierarchy rule, supervision and cancellation rule, lifecycle rule, Coding Conduct Gate, Contract Coverage Gate, debugging integrity gate, and Meta-Cognitive Debug/Repair Gate when it fires: child workers do not ask `coding-agents を使いますか？ [Y/n]` or start independent nested Coding Agents workflows inside a parent-managed scoped assignment; permitted descendant delegation is allowed only inside finite inherited depth; quiet workers are not interrupted before heartbeat deadlines or `no_interrupt_until`; workers that are still running at `heartbeat_interval` self-report completed/current/blocker/ETA progress without parent polling; explicit completed, blocked, or failed worker output bypasses the quiet-worker stale path and triggers immediate collect/integrate/close-or-retire handling; heartbeat and progress telemetry is not completion evidence; stale cancellation follows missed heartbeat -> soft ping/status request -> grace wait -> stale mark -> cancel/replace only if still silent or invalid; subagents return concise integration material, stop waiting for more work, are closed or retired promptly when no longer needed under an allowed cancellation reason, reuse mature GitHub/npm OSS when suitable and approved, analyze bugs from first principles, do not add fallback implementations that hide main-flow errors, do not claim debug completion through log-only, fallback-only, skip-only, or return-to-main-loop-only changes, map accepted decisions and completion conditions to Contract Coverage evidence before completed integration, and include context-impact inspection plus cross-feature checks for gate-required work.

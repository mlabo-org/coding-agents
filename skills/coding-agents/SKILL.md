...
- `<git-root>/.coding-agents/task.md`: current task SSOT, including purpose, scope, semantic `work_type`, non-goals, and completion conditions.
...
- `<git-root>/.coding-agents/runner.md`: conditional operational log for `assign`, `collect`, `run`, `orchestrate`, parent-integration packets, and process results. Runner and integration packets render `feature_profile` as the selected id or `none` and `work_type` as the selected id or `auto`. Create or update it only when that activity occurs. Do not require `runner.md` for intake, specification, documentation-only, or audit flows that have no runner activity.

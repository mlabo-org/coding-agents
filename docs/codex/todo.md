...
## T-008 Runner Activation Verification

- [x] T-008.1 Record source/cache refresh verification for the current runner state.
- [x] T-008.2 Record new-session `@coding-agents` approximation after cache refresh.
- [x] T-008.3 Keep existing runner smoke verification linked to A-009 and `docs/codex/runner.md`.
- [x] T-008.4 Integrate smoke temp cwd result for `/tmp/coding-agents-smoke.DSOu6j`.
